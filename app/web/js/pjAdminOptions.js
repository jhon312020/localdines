...
(function ($, undefined) {
	$(function () {
		...
		$(".onoffswitch-front_qrMenu .onoffswitch-checkbox").change(function() {
			if ($(this).prop("checked")) {
				$cinfo = $.ajax({
				  type: "POST",
				  async: false,
				  url: "index.php?controller=pjAdminQrCode&action=pjActionUpdateFrontOption",
				  data: { 
					value: function() {
					  return 1;
					}
		...
		var validate = ($.fn.validate !== undefined),
			...
			$frmUpdateQrOptions = $('#frmUpdateQrOptions'),
		...
		if ($frmUpdateQrOptions.length && validate) {
			$frmUpdateQrOptions.validate();
		}
		// ---------------------------------------------------------------------
		...
		$("#boxNotificationsWrapper").on("change", 'input[name="recipient"]', function () {
			...
			return false;
		}).on("change", ".onoffswitch-front_qrMenu .onoffswitch-checkbox", function (e) {
			console.log("comes to option");
			if ($(this).prop("checked")) {
				  $cinfo = $.ajax({
				    type: "POST",
				    async: false,
				    url: "index.php?controller=pjAdminOptions&action=pjActionChageFrontOption",
				    data: { 
				      value: function() {
				        return 1;
				      }
		...
		}).on("click", "#frmUpdateQrSettings #btn-save-settings", function() {
			var frmQrSettings = $("#frmUpdateQrSettings");
			var formData = new FormData(frmQrSettings);
			$.ajax({
				data: formData,
				url: "index.php?controller=pjAdminQrCode&action=pjActionUpdate",
				type: "POST",
		...
		}).on("change", "#o_terms_show", function () {
			if ($(this).is(":checked")) {
				$(".boxTerms").removeClass("hidden");
				$('input[name="value-enum-o_terms_show"]').val("1|0::1");
			} else {
				$(".boxTerms").addClass("hidden");
				$('input[name="value-enum-o_terms_show"]').val("1|0::0");
			}
		});
	});
})(jQuery_1_8_2);