	...
	$(function () {
		...
		if ($("#grid").length > 0 && datagrid) 
		{
			var gridOpts = {
					...
					dataUrl: "index.php?controller=pjAdminSms&action=pjActionGetSms",
					dataType: "json",
		...
		$(document).on("submit", ".frm-filter", function (e) {
			...
			$grid.datagrid("load", "index.php?controller=pjAdminSms&action=pjActionGetSms", "id", "ASC", content.page, content.rowCount);
			return false;
			...
			swal({
            	...
            	if(number != '')
        		{
            		...
            		$.post("index.php?controller=pjAdminSms&action=pjActionTestSms", $frmSms.serialize(), function(data) {
        		        if(data.status == 'OK')
        	        	{
			...
			var $this = $(this);
			$.post("index.php?controller=pjAdminSms&action=pjActionVerify", $frmSms.serialize(), function(data) {
		        if(data.status == 'OK')
	        	{
		        	swal({
		                title: data.text,
