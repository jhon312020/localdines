...
class pjAdminSuppliers extends pjAdmin {
    ...
    $this->appendJs('pjAdminSupplier.js');
	}

  public function pjActionGetSupplierList() {
    $this->setAjax(true);
    if ($this->isXHR()) {
      $pjSupplierModel = pjSupplierModel::factory();
    
      $pjSupplierModel = $pjSupplierModel
      ->select("t1.*")
      ->where("is_active=1");

      if ($q = $this->_get->toString('q')) {
          // echo "hello "; exit;
          $pjSupplierModel = $pjSupplierModel->where("(t1.name LIKE '%$q%' OR t2.contact_person LIKE '%$q%')");
      }
  ...
  public function pjActionCreate() {
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_SERVER['CONTENT_LENGTH']) && (int) $_SERVER['CONTENT_LENGTH'] > $post_max_size) {
      pjUtil::redirect(PJ_INSTALL_URL . "index.php?controller=pjAdminSuppliers&action=pjActionIndex&err=AP05");
    }
    $this->checkLogin();
    if (!pjAuth::factory()->hasAccess()) {
      $this->sendForbidden();
    ...
    if (self::isPost()) {
      ...
      if ($post['name'] && $post['contact_person']) {
        ...
        $err = 'AP09';
        $pjSupplierModel->setAttributes($data)->insert()->getInsertId();
      } else {
        $err = 'AP01';
      }
      pjUtil::redirect($_SERVER['PHP_SELF'] . "?controller=pjAdminSuppliers&action=pjActionIndex&err=$err");
    }
    if (self::isGet()) {
      $this->setLocalesData();
      $this->set('company_arr', pjSupplierModel::factory()
          ->select('t1.*')
  ...
  public function pjActionDeleteSupplier() {
    ...
    $pjSupplierModel = pjSupplierModel::factory();
    $arr = $pjSupplierModel->find($this->_get->toInt('id'))->getData();
    if (!$arr) {
      self::jsonResponse(array('status' => 'ERR', 'code' => 103, 'text' => 'Company not found.'));
    }
    $id = $this->_get->toInt('id');
    if ($pjSupplierModel->setAttributes(array('id' => $id))->erase()->getAffectedRows() == 1) {
  ...
  public function pjActionUpdate() {
    ...
    if (self::isPost()) {
      ...
      if ($post['name'] && $post['contact_person'] && $post['address'] && $id) {
        ...
        //$data['updated_date'] = date("Y-m-d H:i:s");
        $err = 'AP09';
        $pjSupplierModel->reset()->where('id', $id)->limit(1)->modifyAll($data);
      } else {
        $err = 'AP01';
      }
      pjUtil::redirect($_SERVER['PHP_SELF'] . "?controller=pjAdminSuppliers&action=pjActionIndex&err=$err");
    ...
    if (self::isGet()) {
      $id = $this->_get->toInt('id');
      $arr = pjSupplierModel::factory()->find($id)->getData();
      if (count($arr) === 0) {
        pjUtil::redirect(PJ_INSTALL_URL. "index.php?controller=pjAdminSuppliers&action=pjActionIndex&err=AP08");
      } 
      $this->set('arr', $arr);
      $this->setLocalesData();
      $this->set('company_arr', pjSupplierModel::factory()
