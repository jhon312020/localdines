...
class pjAdminIncomes extends pjAdmin {
  ...
  public function pjActionGetIncomeList() {
    $this->setAjax(true);
    if ($this->isXHR()) {
      ...
      if ($q = $this->_get->toString('q')) {
          // echo "hello "; exit;
          $pjIncomeModel = $pjIncomeModel->where("(t1.name LIKE '%$q%' OR t2.contact_person LIKE '%$q%')");
      }
  ...
  public function pjActionCreate() {
    ...
    if (self::isGet()) {
      $this->setLocalesData();
      $this->set('company_arr', pjIncomeModel::factory()
          ->select('t1.*')
