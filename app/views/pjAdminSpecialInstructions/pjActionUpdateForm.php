...
?>
<form action="<?php echo $_SERVER['PHP_SELF']; ?>?controller=pjAdminSpecialInstructions&amp;action=pjActionUpdate" method="post" id="frmUpdateSpecialInstruction" enctype="multipart/form-data">
	<input type="hidden" name="category_update" value="1" />
	<input type="hidden" name="id" value="<?php echo $tpl['arr']['id'];?>" />
    <div class="panel-heading bg-completed">
        <p class="lead m-n"><?php echo "Update Special Instruction";?></p>
    </div><!-- /.panel-heading -->
    ...
    <div class="panel-body">
        ...
        <?php if(!empty($tpl['special_instruction_types']))
        {
            ...
            <div class="form-group">
                ...
                <select name="special_instruction_type" id="special_instruction_type" class="form-control select2-hidden-accessible required" data-placeholder="-- <?php __('lblChoose'); ?> --" data-msg-required="<?php __('fd_field_required', false, true);?>">
                    ...
                    {
                        ?><option value="<?php echo $v['id']; ?>" <?php echo $tpl['arr']['parent_id'] ==  $v['id'] ? "selected" : ""; ?>><?php echo stripslashes($v['type']); ?></option><?php
                    }
                    ?>
                </select>
            </div><!-- /.form-group -->
        ...
        if (!empty($tpl['arr']['image']) && is_file(PJ_INSTALL_PATH . $tpl['arr']['image']))
        {
            ...
            <div id="boxExtraImage" class="form-group">
                ...
                <a href="<?php echo $_SERVER['PHP_SELF']; ?>?controller=pjAdminSpecialInstructions&amp;action=pjActionDeleteImage&amp;id=<?php echo pjSanitize::html($tpl['arr']['id']);?>" class="btn btn-xs btn-danger btn-outline btnDeleteImage" data-id="<?php echo pjSanitize::html($tpl['arr']['id']);?>"><i class="fa fa-trash"></i> <?php __('plugin_app_management_btn_delete'); ?></a>
            </div>
            <?php
        } 
        ?>
